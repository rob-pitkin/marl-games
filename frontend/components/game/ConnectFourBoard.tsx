...
import type { AIResponse, MoveResponse, StartGameResponse } from "@/lib/api-client";
...
export function ConnectFourBoard({ onGameEnd }: ConnectFourBoardProps) {
    ...
    function observationToBoard(observation: number[][][], forPlayer: string): CellState[][] {
    ...
    async function handleStartGame() {
        try {
            ...
            const response: StartGameResponse = await apiClient.startGame("connect_four");
            ...
            if (playerOrder === "second") {
                ...
                const aiResponse: AIResponse = await apiClient.getAIMove(response.session_id);
                setBoard(observationToBoard(aiResponse.observation, aiResponse.current_player));
    ...
    async function handleColumnClick(col: number) {
        ...
        try {
            ...
            // Make player move
            const moveResponse: MoveResponse = await apiClient.makeMove(sessionId, col);
            setBoard(observationToBoard(moveResponse.observation, moveResponse.current_player));
            ...
            <div className="space-y-6">
                ...
                {!sessionId && (
                    <div className="flex items-center justify-center gap-4 p-4 bg-muted/50 rounded-lg">
                        ...
                        <Select
                            value={playerOrder}
                            onValueChange={(value) => setPlayerOrder(value as "first" | "second")}
                        >
                            ...
                            <SelectContent>
                                ...
                                <SelectItem value="second">🟡 Second Player (Yellow)</SelectItem>
                            </SelectContent>
                        </Select>
                    </div>
                )}
                        ...
                        >
                            {board.flatMap((row, rowIndex) =>
                                row.map((cell, colIndex) => {
                                    const cellKey = `cell-r${rowIndex}c${colIndex}`;
                                    return (
                                        <button
                                            type="button"
                                            key={cellKey}
